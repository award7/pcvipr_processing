function txt = myupdatefcn_maxVel(empt,event_obj)
% Customizes text of data tips
count = 2;
global maxVel_vol branchList PointLabel branchLabeled Ntxt flowPulsatile_vol fig res Planes p dcm_obj hfull timeMIPcrossection segment1 vTimeFrameave
...
value = maxVel_vol(x,y,z);
for i = 1:2*count+1
    average(i) = maxVel_vol(x2(i),y2(i),z2(i));
end
...
txt = {['Point Label:' , PointLabel , sprintf('\n'), ...
    'Max Velocity: ',sprintf('%0.3f',value),  ' cm/s ', sprintf('\n'), ...
    'Average: ',sprintf('%0.3f',mean(average)), ' cm/s', sprintf('\n'), ...
