function txt = myupdatefcn_flow(empt,event_obj)
% Customizes text of data tips
count = 2;

global flowPerHeartCycle_vol PointLabel branchList branchLabeled Ntxt flowPulsatile_vol res Planes fig p dcm_obj hfull timeMIPcrossection segment1 vTimeFrameave
...
value = flowPerHeartCycle_vol(x,y,z);
for i = 1:2*count+1
    average(i) = flowPerHeartCycle_vol(x2(i),y2(i),z2(i));
end
...
txt = {['Point Label:' , PointLabel , sprintf('\n'), ...
    'Flow: ',sprintf('%0.3f',value),  ' mL/cycle ', sprintf('\n'), ...
    'Average: ',sprintf('%0.3f',mean(average)), ' mL/cycle', sprintf('\n'), ...
